...
@app.post("/Multicity Search Flight")
def Search():
...
@app.post("/Fare Rule Flight")
def Farerule(id:str,flowtype:str,current_user: TokenData = Depends(get_current_user)):
...
def Booking(payload: dict,current_user: TokenData = Depends(get_current_user)):
...
@app.post("/Fare Validate Flight")
def Validate(bookingid:str,current_user: TokenData = Depends(get_current_user)):
...
@app.post("/Confirm Book Flight")
def Confirm(bookingid:str,amount:float,current_user: TokenData = Depends(get_current_user)):
    ...
    if response.ok:
        ...
        return res
#Get Booking Details
@app.post("/Booking Details Flight")
def Bookingdetails(bookingid:str,current_user: TokenData = Depends(get_current_user)):
...
@app.post("/Booking Details with Pax Pricing Flight")
def Bookingdetails(bookingid:str,current_user: TokenData = Depends(get_current_user)):
    url = "https://apitest.tripjack.com/oms/v1/booking-details"
    headers = {'Content-Type': 'application/json','apikey': TJ_KEY}
    ...
    if response.ok:
        ...
        return res
#Release PNR 
@app.post("/Release PNR Flight")
def ReleasePNR(bookingid:str,pnrs:list,current_user: TokenData = Depends(get_current_user)):
    ...
    if response.ok:
        ...
        return res
#Get User Balance
@app.post("/User Balance Flight")
def Userbalance(apikey:str,current_user: TokenData = Depends(get_current_user)):
...
@app.post("/autocomplete_nationality Hotel")
...
@app.post("/autocomplete_city Hotel")
async def autocomplete_city(query: str):
    city = get_city_data()
...
@app.post("/HotelDetails Hotel")
def Details(searchid:str,current_user: TokenData = Depends(get_current_user)):
    headers = {
    'apikey': TJ_KEY,
    'Content-Type': 'application/json',
    'Access-Control-Allow-Origin':'*',
...
def CancellationPolicy(hotelid:str,optionid:str,current_user: TokenData = Depends(get_current_user)):
    headers = {
    'apikey': TJ_KEY,
    'Content-Type': 'application/json',
    'Access-Control-Allow-Origin':'*',
...
@app.post("/Review Hotel")
def Review(hotelid:str,optionid:str,current_user: TokenData = Depends(get_current_user)):
    headers = {
    'apikey': TJ_KEY,
    'Content-Type': 'application/json',
    'Access-Control-Allow-Origin':'*',
...
def CreateTransaction(amount:float):
    ...
    create_transaction_params_BTC = {
            ...
            'currency1' : "USD",
            "currency2" : "BTC",
            'buyer_email' : 'sonisn970@gmail.com'
        }
